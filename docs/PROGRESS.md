...
- Integrated **GitHub Star API** with a 24-hour caching layer in metadata to visualize repository popularity on the home page.
- Added **Real-time Storage Statistics** to the dashboard, tracking disk usage for both documentation and cached source code.
...
- **System Wipe (Nuclear Option)**:
  - Added a global maintenance tool to the Storage API that allows for a complete system reset, purging all tutorials and caches while preserving the directory structure.
  - Integrated a safety-guarded confirmation modal on the Home Page.
...
### Interactive Documentation & Maintenance
- **Mermaid Diagram Integration**:
  - Implemented live rendering of ` ```mermaid ` blocks in the Markdown viewer using `mermaid.js`.
  - Developed a custom dark-themed configuration for diagrams that automatically matches the app's blue and zinc design tokens.
  - Added a post-processing pipeline to strip fixed SVG dimensions, ensuring diagrams are truly responsive and scalable.
- **Advanced Pan & Zoom**:
  - Integrated `react-zoom-pan-pinch` to provide an interactive exploration experience for complex charts.
  - Developed a floating, glassmorphic control panel for Zoom In/Out and View Reset.
  - Implemented infinite-boundary panning (`limitToBounds={false}`) to resolve clipping issues and allow full exploration of large diagrams.
- **Developer Experience (DX) Alignment**:
  - Created a comprehensive `AGENTS.md` (Developer Guide) at the project root to standardize coding patterns, tech stack documentation, and quick-start commands for AI agents and human contributors.
  - Refined the premium design system with improved backdrop blurs and staggered entry animations for a more polished "Pro Tools" feel.

### Export & Sharing Infrastructure
- **Multi-Format Export API**:
  - Developed a specialized `/api/tutorials/[id]/export` endpoint for documentation retrieval.
  - Integrated `JSZip` for batch bundling of Markdown files.
  - Implemented an on-the-fly **PDF Generation Engine** using `Puppeteer` and `marked`, featuring custom CSS for high-quality, academic-style technical documentation.
  - Added an "Export Docs" dropdown to the Tutorial Viewer with support for both `.zip` (Markdown) and PDF formats.
