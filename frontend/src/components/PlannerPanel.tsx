"use client";

import { ChevronDown, ChevronRight } from "lucide-react";
import { Todo } from "@/hooks/useAgentStream";

interface PlannerPanelProps {
    todos: Todo[];
    isLoading: boolean;
    isCollapsed: boolean;
    onToggleCollapse: () => void;
}

export function PlannerPanel({ todos, isLoading, isCollapsed, onToggleCollapse }: PlannerPanelProps) {
    const completed = todos.filter(t => t.status === "completed").length;
    const progress = todos.length > 0 ? (completed / todos.length) * 100 : 0;

    return (
        <aside className="h-full flex flex-col border-r border-zinc-900 bg-black">
            {/* Header / Toggle Bar */}
            <div
                className="p-3 border-b border-zinc-900 cursor-pointer hover:bg-zinc-900/50 transition-colors bg-[#0A0A0A]"
                onClick={onToggleCollapse}
            >
                <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-white flex items-center gap-2">
                        {isCollapsed ? <ChevronRight size={16} className="text-zinc-500" /> : <ChevronDown size={16} className="text-zinc-500" />}
                        The Plan <span className="text-zinc-500 font-normal">({todos.length > 0 ? `${completed}/${todos.length}` : "TODOs"})</span>
                    </h3>
                    {!isCollapsed && todos.length > 0 && (
                        <span className="text-zinc-500 text-xs font-mono">
                            {Math.round(progress)}%
                        </span>
                    )}
                </div>
                {/* Progress bar - Only show when expanded or if you want a tiny peak? User said shrink to top, so let's hide it in collapsed */}
                {!isCollapsed && todos.length > 0 && (
                    <div className="mt-2 h-1 bg-zinc-800 rounded-full overflow-hidden">
                        <div
                            className="h-full bg-gradient-to-r from-emerald-500 to-emerald-400 transition-all duration-500 ease-out"
                            style={{ width: `${progress}%` }}
                        />
                    </div>
                )}
            </div>

            {!isCollapsed && (
                <div className="flex-1 overflow-y-auto p-4 scrollbar-hide bg-black">
                    {todos.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center p-4">
                            {isLoading ? (
                                <div className="space-y-4">
                                    <div className="w-12 h-12 border-2 border-blue-500/30 border-t-blue-500 rounded-full animate-spin mx-auto"></div>
                                    <div className="text-sm text-zinc-500">
                                        Creating plan<span className="typing-cursor"></span>
                                    </div>
                                </div>
                            ) : (
                                <>
                                    <div className="text-sm text-zinc-500">No tasks yet</div>
                                </>
                            )}
                        </div>
                    ) : (
                        <div className="space-y-2 stagger-children flex flex-col justify-start h-full pb-4">
                            {todos.map((todo, index) => (
                                <div
                                    key={index}
                                    className={`flex items-start gap-2.5 p-2 rounded-lg transition-all border ${todo.status === "completed"
                                        ? "bg-[#0A0A0A] border-zinc-900/50"
                                        : todo.status === "in_progress"
                                            ? "bg-amber-500/5 border-amber-500/30 shadow-[0_0_15px_rgba(245,158,11,0.05)]"
                                            : "bg-[#0A0A0A] border-zinc-900/50"
                                        }`}
                                >
                                    {/* Status icon */}
                                    <div className="mt-0.5 flex-shrink-0">
                                        {todo.status === "completed" ? (
                                            <div className="w-4 h-4 bg-zinc-800 border border-zinc-700 rounded-full flex items-center justify-center shadow-sm">
                                                <svg className="w-2.5 h-2.5 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                                                </svg>
                                            </div>
                                        ) : todo.status === "in_progress" ? (
                                            <div className="w-4 h-4 border-2 border-amber-500/50 rounded-full animate-pulse shadow-lg shadow-amber-500/20"></div>
                                        ) : (
                                            <div className="w-4 h-4 border-2 border-zinc-800 rounded-full"></div>
                                        )}
                                    </div>

                                    {/* Task content */}
                                    <span
                                        className={`text-xs flex-1 leading-relaxed ${todo.status === "completed"
                                            ? "text-zinc-600 line-through"
                                            : todo.status === "in_progress"
                                                ? "text-amber-400/90 font-medium"
                                                : "text-zinc-400"
                                            }`}
                                    >
                                        {todo.content}
                                    </span>
                                </div>
                            ))}
                        </div>
                    )}
                </div>
            )}
        </aside>
    );
}
